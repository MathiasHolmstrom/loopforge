...
def test_worker_backend_tolerates_missing_action_type() -> None:
    ...
    snapshot = FileMemoryStore(Path(".")).load_snapshot if False else None  # noqa: F841
...
def test_routing_executor_ignores_action_label_when_execution_steps_are_present(
    tmp_path,
) -> None:
    from loopforge.core.agentic_execution import GenericExecutionPlanExecutor
...
def test_routing_executor_uses_metadata_only_autonomous_mode_for_unbound_actions(
    tmp_path,
) -> None:
    spec = build_spec(
        allowed_actions=[
...
def test_stream_completion_accumulates_chunks() -> None:
    ...
    original_completion = None  # noqa: F841
